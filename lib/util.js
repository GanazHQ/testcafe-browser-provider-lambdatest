...
const promisify = fn => (0, _pify.default)(fn, _pinkie.default);

const request = promisify(_request2.default, _pinkie.default);
...
async function _parseCapabilities(id, capability) {
  try {
    ...
    const parseCapabilitiesData = (0, _desiredCapabilities.default)(capability)[0];
    ...
    if (PROCESS_ENVIRONMENT.LT_TUNNEL_NAME) capabilities[id].tunnelName = PROCESS_ENVIRONMENT.LT_TUNNEL_NAME;else {
      try {
...
function _saveFile(screenshotPath, base64Data) {
  return new _pinkie.default((resolve, reject) => {
    _fs.default.writeFile(screenshotPath, base64Data, 'base64', err => err ? reject(err) : resolve());
  });
}

function _getAdditionalCapabilities(filename) {
  return new _pinkie.default((resolve, reject) => {
    _fs.default.readFile(filename, 'utf8', (err, data) => err ? reject(err) : resolve(JSON.parse(data)));
  });
}

function sleep(ms) {
  return new _pinkie.default(resolve => {
